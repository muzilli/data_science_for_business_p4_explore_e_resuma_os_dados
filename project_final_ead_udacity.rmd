...
names(labels_graphs_variables)[13] <- "total.acidez"
```
...
# Buscar o nome da label para o grafico apartir da chave 
...
# Gerar todos os gráficos para visualizacao de um dataframe
#(utilizar lapply para este funcao)

generate_graph_univariate_by_dataframe <- function(data, 
                                                   key_x, 
                                                   ylabel = lbl_qtd_registros){
...
generate_graph_univariate_boxplot_by_dataframe <- function(data, 
                                                           key_x, 
                                                           ylabel ){
  xlabel <- get_graph_label(key_x)
...
generate_graph_bivariate_x_as_quality_by_dataframe <- function(data, 
                                                               key_y, 
                                                               ylabel ){
  if (key_y != 'quality') {
...
red_wine_quality$quality.classification <- 
  factor(red_wine_quality$quality.classification,
                         levels = c(0, 1) ,
                         labels = c("Ruim", "Bom"))

red_wine_quality$quality.factor <- ordered(
    red_wine_quality$quality,levels = seq(0, 10, 1)
  )
...
# numero de plots para 12
num_plots <- 12

# cria variavel com todos os graficos
plots_variables <- lapply(colnames(red_wine_quality)[0:num_plots], 
      generate_graph_univariate_by_dataframe, 
...
```{r fig.height = 5, Univariate_Plots_qualidade}
# graficio 12 de qualidade 
...
```{r fig.height = 7, Univariate_Plots_qualidade_summarization}
...
ggplot(red_wine_quality,aes(x=quality.classification)) +
  ...
  xlab(lbl_qualidade_cls) +
  ylab(lbl_qtd_registros) +
  ggtitle("Quantidade de Vinhos Bons e Ruins")
```

> Conforme verificado nos dados referênte a qualidade do vinho a maioria dos vinhos são de qualidade normal pois a 
maior parte das observações estão entre 5 e 7, e a mediana é 6, dessa forma posso rotular os vinhos de forma a criar 
uma nova variável para auxiliar na próximas análises, devido a concentração dos dados estarem entre 5 e 6 será criado 
uma nova variavel __quality.classification__, onde _'Ruim'_ será para vinhos de baixa qualidade onde possuem qualidade 
abaixo de 6 e _'Bom'_ de boa qualidade com qualidade superior ou igual à 6.

> É possível ver com a nova classificação que possui mais vinhos de boa qualidade do que de baixa qualidade

> Deixei a distribuição logaritmica no eixo y pois ficou muito mais simples para visualizar a amostragem da qualidade.

##### Análise inicial de Acidez

> Gráfico de Acidez Fixa

```{r fig.height = 5, Univariate_Plots_acidez_fixa}
#grafico 1 de acidez fixa
...
```{r fig.height = 5, Univariate_Plots_acidez_volatil}
# grafico 2 de acidez volatil
...
> Conforme observação do autor do dataset na descrição de atributo quanto maior o a acidez volátil mais desagradável é o 
vinho, sendo assim o dado de acidez volátil é um dado que pode impactar a qualidade do vinho.

> Com base no [Clube do Vinho](https://www.clubedosvinhos.com.br/acidez-no-vinho-nao-tema/) 
|Vinhos tintos e brancos tem resultados diferentes frente a acidez. Tintos mais ácidos são mais elegantes, enquanto brancos 
são mais refrescantes.

> Com base na [Revista Adega](https://revistaadega.uol.com.br/artigo/o-alcool-e-a-acidez_6055.html)
O álcool e acidez são elementos chave do vinho de qualidade. O seu papel é tão fundamental que própria decisão de iniciar 
vindima é condicionada pelo equilíbrio entre açúcares (que se tornarão álcool após fermentação) e acidez existentes nas uvas.

> Com base nos gráficos é possível verificar que a acidez está equilibrada entre as observações, pois a maior parte das observações 
estão entre o primeiro quartil e terceiro quartil, da acidez fixa estão entre o mínimo de 7.10 e o terceiro quartil com 9.20, 
e a acidez volátil está entre o mínimo de 0.39 e o terceiro quartil de 0,64.

> Ambas as distribuições acidez fixa e acidez volátil possuem caudas positivas longas, e suas médias são mais alta do que suas medianas, 
e tornam a mediana uma melhor medida do valor central. Além disso, a distribuição de acidez volátil tem pequena distribuição bimodal.
...
```{r fig.height = 5, Univariate_Plots_acido_acucar}
#grafico 4 acucar residual
p4 <- plots_variables[[4]] + scale_x_continuous(breaks = seq(0, 16, 1))
p4.log10x <- p4 + scale_x_log10() 
multiplot(p4, cols = 2)
...
```{r fig.height = 5, Univariate_Plots_acido_cloreto}
#grafico 5 cloretos
p5 <- plots_variables[[5]] + scale_x_continuous(breaks = seq(0, 0.65, 0.05))
p5.log10x <- p5 + scale_x_log10() 
multiplot(p5, cols = 2)
...
```{r fig.height = 5, Univariate_Plots_acido_citrico}
# grafico 3 acidos citrico
...
> O ácido cítrico parece uma distribuição bimodal, e aparenta poucos valores discrepantes pelo gráfico, 50% dos valores estão entre 0.090 
e 0.420 e a media está em 0.260
...
```{r fig.height = 5, Univariate_Plots_enxofre_livre}
#grafico 6 dioxido de enxofre livre
p6 <- plots_variables[[6]] + scale_x_continuous(breaks = seq(0, 72, 5))
p6.log10x <- p6 + scale_x_log10() 
multiplot(p6, p6.log10x, cols = 2)
...
```{r fig.height = 5, Univariate_Plots_enxofre_total}
# grafico 7 dioxido de enxofre total
p7 <- plots_variables[[7]] + scale_x_continuous(breaks = seq(0, 300, 15))

p7.log10x <- p7 + scale_x_log10() 
multiplot(p7, p7.log10x, cols = 2)
...
```{r fig.height = 5, Univariate_Plots_sulfato}
# grafico de sulfato de potassio
p10 <- plots_variables[[10]] + scale_x_continuous(breaks = seq(0, 2, 0.1))

multiplot( p10, cols = 1)

paste("Summary de Sulfatos")
summary(red_wine_quality$sulphates)
```

> Conforme descriçoes do autor para dióxido de enxofre e sulfato de potácio, atuam como antimicrobial e antioxidantes, o dióxido de enxofre total, 
pode interferir no sabor e no aroma.
...
```{r fig.height = 5, Univariate_Plots_densidade}
# grafico de densidade
p8 <- plots_variables[[8]] 
p8 <- p8 + scale_x_continuous(breaks = seq(0.900, 1.010, 0.001))
...
> A distribuição de alcool estão entre mínimo de 8.40 e 14.90, onde  50% das observações estão entre 9.50 e 11.10, a mediana é de 10.20 
e a média é de 10.42

##### Análise inicial de pH

> Gráfico de pH

```{r fig.height = 5, Univariate_Plots_ph}
# grafico de pH
p9 <- plots_variables[[9]] + scale_x_continuous(breaks = seq(0.00, 4.01, 0.08))
...
... Nos vinhos tintos, níveis de pH muito baixos estão acompanhados por uma cor roxa brilhante. Assim que o pH vai aumentando, 
sombras azuladas aparecerão. Em níveis altos de pH, reflexos marrons ficam evidentes ... 
...
> 1 atributo de saída __qualidade__ ( qualidade dos vinhos tintos ), a análise princial será entender quais propriedades qímicas 
influenciam na qualidade dos vinhos tintos.
...
> A principal característica do conjunto de dados é a qualidade, a idéia principal é tentar avaliar a o comportamento dos demais atributos 
interferem na qualidade do vinho com alguns outros recursos, utilizando análise bivariada e multivariada.

### Quais outros atributos você acha que podem lhe auxiliar na investigação destes atributos de interesse?

> Eu aprendi, olhando alguns artigos na internet especializados em vinho como está em itens acima da análise, e até mesmo no [link](https://revistaadega.uol.com.br/artigo/o-alcool-e-a-acidez_6055.html) uma boa qualidade do vinho pode ser dada pelo alcool (alcool), 
gosto ácido (ácidos) e o dioxido de enxofre total pode influenciar no aroma do vinho, portanto pode modificar a qualidade do vinho, 
portanto podem ser atributos que podem ser relevantes a serem analisados como acidez fixa, acidez volátil, ácido cítrico, dióxido de enxofre 
álcool e qualidade.
...
> quality.classification para auxiliar em uma classificação mais simples da qualidade de vinhos do especialista se a qualidade for manor que 
6 classifiquei como Ruim = 0 e caso seja maior ou igual a 6 classifiquei como Bom = 1

> total.acidity para auxiliar em um calculo de total de acidez do vinho com o total de ácidos do vinho somando a acidez fixa, a acidez volatil 
e o ácido citrico
...
> O açucar residual e os cloretos possuem distribuições incomuns e com maior propenções a terem outliers, 
mesmo com análise em log10 aplicado possui áreas sem valor com valores distantes e com cauda longa e bimodal.
...
# cria variavel com todos os graficos
plots_variables <- lapply(colnames(red_wine_quality)[0:11], 
                          generate_graph_bivariate_x_as_quality_by_dataframe, 
                          data = red_wine_quality)
...
ggplot(aes(x = alcohol, 
           y = density), 
       data = red_wine_quality) +
  geom_point(color = I('darkblue'),
             alpha = 0.5, 
             size = 1, 
             position = 'jitter') +
  coord_cartesian() +
  scale_x_continuous(breaks = seq(8, 15, 0.5)) +
  scale_y_continuous(breaks = seq(0.00, 2.00, .001)) +
  xlab(labels_graphs_variables['alcohol']) +
  ylab(labels_graphs_variables['density']) +
...
ggplot(aes(x = total.acidity, y = quality), 
       data = red_wine_quality) +
  geom_point(color = I('darkblue'), 
             alpha = 0.5, 
             size = 1, 
  ...
  coord_cartesian(xlim = 
                    c(quantile(red_wine_quality$total.acidity, .01),
                      quantile(red_wine_quality$total.acidity, .99)
                      )
                  ) +
  scale_x_continuous(breaks = seq(0, 18, .5))+
  scale_y_continuous(breaks = seq(0, 10, 1)) +
  xlab(labels_graphs_variables['total.acidez']) +
  ylab(labels_graphs_variables['quality']) +
  geom_smooth(method = "lm", se = FALSE) +
...
qplot(total.acidity, 
      data = red_wine_quality, 
      binwidth = .1, fill = quality.factor) +
...
> Avaliando o gráfico acima e os dados da tabela acima é possível identificar que os vinhos tanto de boa qualidade quanto qualidade intermediaria de 5 para cima possuem alcool acima de 7.

```{r Bivariate_Plots_acidez_citrica_vs_acidez_volatil}
ggplot(aes(x = citric.acid, y = volatile.acidity), 
  ...
  geom_point(color = I('darkblue'), 
             ...
             position = 'jitter') +
  coord_cartesian(xlim = 
                    c(quantile(red_wine_quality$citric.acid, .01),
                      quantile(red_wine_quality$citric.acid, .99)
                      )
                  ) +
  scale_y_continuous(breaks = seq(0, 2, 0.1)) +
  xlab(labels_graphs_variables['citric.acid']) +
  ylab(labels_graphs_variables['volatile.acidity']) +
  scale_x_continuous(breaks = seq(0, 1, .05)) +
  geom_smooth(method = "lm", se = FALSE) +
  ggtitle('Acido cítrico pela Acidez volátil')
```

> Neste gráfico é possível identificar que  a correlação do ácido cítrico e da acidez volátil é descendente e possui uma correlação razoalvelmente forte.

```{r Bivariate_Plots_alcool_vs_qualidade}
qplot(alcohol, 
      data = red_wine_quality, 
      binwidth = .1, 
      fill = as.factor(quality.classification)) +
  coord_cartesian() +
  labs(fill = lbl_qualidade_cls) +
...
> É possível identificar que quanto mais o alcool aumenta a qualidade do vinho aumenta, isso tanbem pode identificar pois a mediana dos vinnhos bons é de 10.80.
...
qplot(density, 
  ...
  coord_cartesian() +
  labs(fill = lbl_qualidade_cls) +
  xlab(labels_graphs_variables['density']) +
  ylab(lbl_qtd_registros) +
  scale_x_continuous(breaks = seq(0, 1.0037, 0.0015))+
...
```
 ...
 > Verificando as densidades quanto mais vai subindo a densidade mais vai diminuindo a qualidade
...
> Já com a acidez total quando chega na casa de 11 fica estabilizada
...
qplot(sulphates, 
  ...
  coord_cartesian() +
  labs(fill = lbl_qualidade_cls) +
  xlab(labels_graphs_variables['sulphates']) +
  ylab(lbl_qtd_registros) +
...
> Os sulfatos apesar de ter uma amostra muito grande e uma grande concentracao entre 0,5 e 0,8, os vinhos bons a mediana possui uma mediana maior de sulfato do que os vinhos com uma qualidade pior.
...
qplot(total.sulfur.dioxide, 
  ...
  coord_cartesian() +
  labs(fill = lbl_qualidade_cls) +
  xlab(labels_graphs_variables['total.sulfur.dioxide']) +
  ylab(lbl_qtd_registros) +
...
> O Dioxido de enxofre total é o contrário a mediana de vinhos ruins esta  45.00 e é 26% maior que a media para os vinhos bons
...
ggplot(aes(x = density, 
           y = alcohol, 
           color = as.factor(quality.classification)), 
       data = red_wine_quality) +
  ...
  labs(color = lbl_qualidade_cls) +
  xlab(labels_graphs_variables['density']) +
  ylab(labels_graphs_variables['alcohol']) +
  scale_x_continuous(breaks = seq(0.900, 1.010, 0.001)) +
...
ggplot(aes(x = total.acidity, 
  ...
  xlab(labels_graphs_variables['total.acidez']) +
  ylab(labels_graphs_variables['alcohol']) +
  scale_x_continuous(breaks = seq(5, 18, .5))+
  scale_y_continuous(breaks = seq(0, 15,.5)) +
  geom_smooth(method = "lm", se = FALSE) +
  ggtitle('Alcool pela Acidez Total com a Qualidade')
```
 
> NEste grafico é possível avaliar como a dispersão de qualidade sobre o vimho estão bem centralizados no total de ácidos entre  os valores de 7 e 10,e uma pequena variacao nos demais valores,  e no eixo do alcool esta bem disperso, avalaindo que possui vinhos de todas os teores alcoolicos e bons e ruins.
...
ggplot(aes(x = alcohol, 
  ...
  labs(color = lbl_qualidade_cls) +
  xlab(labels_graphs_variables['alcohol']) +
  ylab(labels_graphs_variables['sulphates']) +
  scale_x_continuous(breaks = seq(0, 15, .5))+
...
> Avaliando os vinhos em questão de alcool pelo sulfato,  existe uma grande concentração  de vinhos com sulfatos bons e ruins com sulfatos  entre 0.4 e 0.8, tanto q as retas de regressão estão quase paralelas, e as dispersões de  alcool não estão com dispersões muido distantes.
...
ggplot(aes(x = quality, 
  ...
  labs(color = labels_graphs_variables['alcohol']) +
  scale_y_continuous(breaks = seq(0.00, 2.00, .001)) +
  xlab(labels_graphs_variables['quality']) +
  ylab(labels_graphs_variables['density']) +
  ggtitle('Qualidade vs Densidade pelo Alcool')
```

> Com analise de do grafico de dispersao deposso ver que esta com uma dispersão bimodal em qualidade 5 e 6 com maior concentração de alcool em qualidade e com maior concentração de alcool.
...
ggplot(aes(x = sulphates, 
  ...
  labs(color = labels_graphs_variables['alcohol']) +
  scale_x_continuous(breaks = seq(0, 2, 0.1)) +
  scale_y_continuous(breaks = seq(0, 2, .001)) +
  xlab(labels_graphs_variables['sulphates']) +
  ylab(labels_graphs_variables['density']) +
...
tree <- rpart(quality.classification ~ 
                alcohol +
                density +
                fixed.acidity +
                volatile.acidity +
                ...
                sulphates +
                total.sulfur.dioxide, 
              data = red_wine_quality, method = "class")
summary(tree)
rpart.plot(tree)
...
> __Alcool pela classificação de qualidade__

```{r Primeiro Grafico Final Bivariate_Plots_total_alcool_vs_qualidade}
qplot(alcohol, 
      data = red_wine_quality, 
      binwidth = .1, 
      fill = as.factor(quality.classification)) +
...
> Quando falamos de qualidade em vinho, temos que levar em consideração uma infinidade de sabores, consistência, aroma, cores e longevidade. No entanto, há apreciadores que defendem a ideia de que o teor alcoólico é o grande medidor para afirmar se um vinho é bom ou não. 
O vinho é composto por mais de 400 substâncias, entre elas o álcool que é o resultado da fermentação entre a glicose e a frutose das uvas. 
De acordo com o que observamos no gráfico acima quanto mais álcool mais saboroso e com uma qualidade maior será o vinho.
...
> __Alcool pela Acidez Total com a Qualidade__

```{r Terceiro Grafico Final Multivariate_Plots_acidez_total_vs_alcool_com_qualidade}
ggplot(aes(x = total.acidity, 
           y = alcohol, 
           color = as.factor(quality.classification)), 
       data = red_wine_quality) +
  ...
  scale_x_continuous(breaks = seq(5, 18, .8)) +
  scale_y_continuous(breaks = seq(0, 15, .5)) +
  labs(color = lbl_qualidade_cls) +
  xlab(labels_graphs_variables['total.acidez']) +
  ylab(labels_graphs_variables['alcohol']) +
...
> Este último gráfico analisamos duas qualidades para o vinho: a vermelha para uma qualidade de um vinho Bom e a verde para uma qualidade Ruim. 
Para vinhos de qualidade boa, quanto menor for a acidez, maior será o grau alcóolico em sua composição.
Já para os vinhos considerados ruins esse número permaneceu na média. 
Lembramos que analisamos a qualidade com variáveis na forma de vetores multivariados. 
...
> __Arvore de Decisão__

```{r Segundo Grafico Final arvore_de_decisa}
tree <- rpart(quality.classification ~ 
                alcohol + 
                density + 
                fixed.acidity +
...
> Conforme observado no gráfico acima, segundo as 5 variáveis com maior importância (alcohol, sulphates, density, volatile.acidity e citric.acid) o álcool tem uma grande parcela na divisão destes itens importantes (quase 50% no total).
Sempre na medida, o álcool é bastante importante para preservar a essência do vinho, tanto enquanto bebida, como um fator sociável de quem o aprecia. Sua presença e influência no sabor macio e adocicado de um vinho sempre será na proporção contrária da presença ácida e tanina que um tipo vinícola pode ter. Tanto que a acidez é colocada no gráfico como um item importante na divisão entre os tipos de sulfatos (não alcóolicos) demonstrado acima. 
Um bom vinho, seja ele branco ou tinto, sempre terá uma acidez equilibrada.
...
> O que pude aprender com esta análise e com estudo de materiais externos é que vinhos com graduações alcoólicas semelhantes também terão perfis parecidos. É claro que eles não serão exatamente os mesmos, mas certamente estão inseridos em grupos bastante próximos uns aos outros. 
...
https://docs.google.com/document/d/1jX3vzkFuFOBGUrlcQ_Lc3jEZVlC_2yyk3tFIbwAI5GQ/edit
https://www.kaggle.com/uciml/red-wine-quality-cortez-et-al-2009

__Exemplos Udacity__

http://adv-r.had.co.nz/Style.html
https://s3.amazonaws.com/content.udacity-data.com/courses/ud651/diamondsExample_2016-05.html

__Revisas e temas sobre vinho:__

https://www.clubedosvinhos.com.br/acidez-no-vinho-nao-tema/
https://revistaadega.uol.com.br/artigo/o-alcool-e-a-acidez_6055.html
https://revistaadega.uol.com.br/artigo/a-importancia-do-ph-no-vinho_1552.html

__R documentation:__

https://ggplot2.tidyverse.org/reference/

__Materiais Adicionais:__

Apostilas do MBA De Machine Learning e Inteligencia Artificial da Universidade FIAP de:
R para Inteligencia Artificial
Estatistica para inteligencia artificial